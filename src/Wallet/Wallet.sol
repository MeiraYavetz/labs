...
contract Wallet {
    ...
    //    require(owner == msg.sender, "Only the owner can update"); //only owner can update gabaaim
        require(gabaim[oldGabai]==1,"the old gabai is not exist"); // check if gabbai exist in my hash
        require(gabaim[newGabai]==0,"the gabai is exist"); // check if gabbai exist in my hash
        gabaim[newGabai]=1;
        gabaim[oldGabai]=0;
    }
